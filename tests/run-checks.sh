...
# create-env refuses to run inside this checkout and writes its generated ops
# files into $PWD, so every check below runs from a scratch directory. One
# mktemp root per invocation, removed whole, so cleanup never globs over paths
# this run does not own.
create_env_dir=$(mktemp -d "${TMPDIR:-/tmp}/bosh-deployment-create-env-check.XXXXXX")
